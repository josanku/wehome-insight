# AWS 배포 가이드 · wehome Studio

위홈 AWS 계정에 `insight.wehome.me` 배포하는 4가지 옵션. **App Runner 추천**.
...
### 커스텀 도메인 연결
App Runner Console → 서비스 → "Custom domains" → Add → `insight.wehome.me`
...
Route 53에서 `insight.wehome.me` A 레코드(ALIAS) → EB 환경 ELB로.
...
Route 53를 쓰면 Apex(`wehome.me`)나 서브도메인(`insight.wehome.me`) 모두 ALIAS로 깔끔하게 연결.
